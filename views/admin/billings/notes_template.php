...
<div class="col-md-12 note-item" data-noteid="<?php echo $note['id']; ?>">
   ...
   <?php if (($note['staffid'] == get_staff_user_id()) || is_admin()) { ?>
      <a href="#" class="pull-right" onclick="remove_billing_note(<?php echo $note['id']; ?>); return false;">
      <i class="fa fa-times text-danger"></i>
         <a href="#" class="pull-right mright5" onclick="toggle_billing_note_edit(<?php echo $note['id']; ?>);return false;"><i class="fa fa-pencil-square"></i></a>
      </a>
   <?php } ?>
   <div class="media-body">
      ...
      <br />
      <div data-billing-note="<?php echo $note['id']; ?>">
         <?php echo check_for_links($note['content']); ?>
      </div>
      <div data-billing-note-edit-textarea="<?php echo $note['id']; ?>" class="hide mtop15">
         <?php echo render_textarea('note-content','',$note['content']); ?>
         <?php if($note['staffid'] == get_staff_user_id() || is_admin()){ ?>
         <div class="text-right">
            <button type="button" class="btn btn-default" onclick="toggle_billing_note_edit(<?php echo $note['id']; ?>);return false;"><?php echo _l('cancel'); ?></button>
            <button type="button" class="btn btn-info" onclick="edit_billing_note(<?php echo $note['id']; ?>);"><?php echo _l('update_note'); ?></button>
         </div>
         <?php } ?>
      </div>
   </div>
