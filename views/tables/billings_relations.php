...
$aColumns = [
    '' . db_prefix() . 'billings.id as id',
    'subject',
    'total',
    'date',
    'open_till',
    '(SELECT GROUP_CONCAT(name SEPARATOR ",") FROM ' . db_prefix() . 'taggables JOIN ' . db_prefix() . 'tags ON ' . db_prefix() . 'taggables.tag_id = ' . db_prefix() . 'tags.id WHERE rel_id = ' . db_prefix() . 'billings.id and rel_type="billing" ORDER by tag_order ASC) as tags',
    'datecreated',
    'status',
    ];
...
$aColumns = hooks()->apply_filters('billings_relation_table_sql_columns', $aColumns);
...
foreach ($rResult as $aRow) {
    ...
    $numberOutput = '<a href="' . admin_url('billings/list_billings/' . $aRow['id']) . '">' . format_billing_number($aRow['id']) . '</a>';

    $numberOutput .= '<div class="row-options">';

    $numberOutput .= '<a href="' . site_url('billing/' . $aRow['id'] . '/' . $aRow['hash']) . '" target="_blank">' . _l('view') . '</a>';
    if (has_permission('billings', '', 'edit')) {
        $numberOutput .= ' | <a href="' . admin_url('billings/billing/' . $aRow['id']) . '">' . _l('edit') . '</a>';
    }
    $numberOutput .= '</div>';

    $row[] = $numberOutput;

    $row[] = '<a href="' . admin_url('billings/list_billings/' . $aRow['id']) . '">' . $aRow['subject'] . '</a>';
