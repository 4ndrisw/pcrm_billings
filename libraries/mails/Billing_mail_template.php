...
class Billing_mail_template
{
    ...
    private function get_language($email, $template = null, $params = [])
    {
        ...
        if ($rel_type != 'billing' && $rel_type != 'lead') {
        ...
        } elseif ($rel_type == 'billing') {
            $this->ci->db->select('rel_type, rel_id');
            $this->ci->db->where('id', $this->get_rel_id());
            $billing = $this->ci->db->get(db_prefix() . 'billings')->row();
            if ($billing && $billing->rel_type == 'lead') {
                $this->ci->db->select('default_language')
                ->where('id', $billing->rel_id);

                $lead = $this->ci->db->get(db_prefix() . 'leads')->row();
            } elseif ($billing && $billing->rel_type == 'customer') {
                $customerDefault = get_client_default_language($billing->rel_id);
    ...
    {
        include_once(billing_get_mail_template_path($className, $params));
